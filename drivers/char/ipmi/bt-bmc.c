...
static irqreturn_t bt_bmc_irq(int irq, void *arg)
{
	...
	rc = regmap_read(bt_bmc->map, bt_bmc->offset + BTCR2, &reg);
	if (rc)
		return IRQ_NONE;
	...
	/* ack pending IRQs */
	regmap_write(bt_bmc->map, bt_bmc->offset + BTCR2, reg);
			     ...
			     struct platform_device *pdev)
{
	 ...
	 */
	rc = regmap_update_bits(bt_bmc->map, bt_bmc->offset + BTCR1,
				(BTCR1_IRQ_EN_H2B | BTCR1_IRQ_EN_HBUSY),
				(BTCR1_IRQ_EN_H2B | BTCR1_IRQ_EN_HBUSY));
...
static int bt_bmc_probe(struct platform_device *pdev)
{
	...
	regmap_write(bt_bmc->map, bt_bmc->offset + BTCR0,
		     FIELD_PREP(BTCR0_IO_BASE, bt_bmc->io_addr) |
